...
export default function CustomerReviews() {
  ...
  const filteredAndSortedReviews = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(review => {
        ...
        const venueName = review.hospitality_venue?.venue_name?.toLowerCase() || '';
  ...
  const displayStats = useMemo(() => {
    ...
    const replied = filteredAndSortedReviews.filter(r => r.venue_reply).length;
    const pending = total - replied;
    
    let avgRating = 0;
    if (total > 0) {
      const totalRating = filteredAndSortedReviews.reduce((sum, r) => sum + (r.rate?.rate || 0), 0);
  ...
  const handleReplySubmit = async (replyText) => {
    ...
    try {
      const result = await reviewService.replyToReview(selectedReview.id, replyText);
  ...
  const renderStars = (rating) => {
    ...
    return Array.from({ length: 5 }, (_, i) => (
      <svg
        key={i}
        className={`w-3 h-3 ${i < rateValue ? 'text-[#FDA800]' : 'text-gray-500'}`}
        fill="currentColor"
        viewBox="0 0 20 20"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-[#2A2A2A] p-4 rounded-lg border border-[#00C1C9]/30">
            ...
            <p className="text-2xl font-bold text-white">
              {searchTerm || filterRating ? displayStats.total : displayStats.globalTotal}
            </p>
            {(searchTerm || filterRating) && displayStats.total !== displayStats.globalTotal && (
              <p className="text-xs text-gray-500 mt-1">of {displayStats.globalTotal} total</p>
            )}
          </div>
          <div className="bg-[#2A2A2A] p-4 rounded-lg border border-[#00C1C9]/30">
            <p className="text-gray-400 text-sm">Average Rating</p>
            <p className="text-2xl font-bold text-[#FDA800]">
              {searchTerm || filterRating ? displayStats.avgRating : displayStats.globalAvgRating} ⭐
            </p>
          </div>
          <div className="bg-[#2A2A2A] p-4 rounded-lg border border-[#00C1C9]/30">
            <p className="text-gray-400 text-sm">Replied</p>
            <p className="text-2xl font-bold text-green-500">{displayStats.replied}</p>
          </div>
          <div className="bg-[#2A2A2A] p-4 rounded-lg border border-[#00C1C9]/30">
            <p className="text-gray-400 text-sm">Pending</p>
            <p className="text-2xl font-bold text-yellow-500">{displayStats.pending}</p>
          </div>
        </div>
        ...
        {currentReviews.length > 0 ? (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-full ${getAvatarColor(getUserDisplayName(review.user))} flex items-center justify-center text-white text-xs font-semibold`}>
                      {getInitials(getUserDisplayName(review.user))}
                    </div>
                    <span className="text-xs text-gray-300 font-medium truncate max-w-[100px]" title={getUserDisplayName(review.user)}>
                ...
                {review.venue_reply ? (
                  <div className="p-2 bg-[#00C1C9]/10 rounded border border-[#00C1C9]/30">
                    <p className="text-xs text-gray-300 mb-1">
                      <span className="font-semibold text-[#00C1C9]">Your Reply:</span>
                    </p>
                    <p className="text-xs text-gray-300 break-words">{review.venue_reply}</p>
          ...
          <div className="text-center text-gray-400 py-12 bg-[#2A2A2A] rounded-lg">
            ...
            <p className="text-lg font-medium mb-2">
              {searchTerm || filterRating > 0 ? 'No reviews found' : 'No reviews yet'}
            </p>
            <p className="text-sm">
          ...
          <div className="flex items-center justify-center gap-2 bg-[#2A2A2A] p-4 rounded-lg">
            ...
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <polyline points="15 18 9 12 15 6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></polyline>
              </svg>
            </button>
            ...
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <polyline points="9 18 15 12 9 6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></polyline>
              </svg>
            </button>
