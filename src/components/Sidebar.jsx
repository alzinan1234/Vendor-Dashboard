...
const navItems = [
  ...
  { name: 'Manage New Reservation', href: '/admin/manage-new-reservation', icon: FileText },
  { name: 'Orders', href: '/admin/orders', icon: BookText },
  { name: 'Earning', href: '/admin/earning', icon: Wallet },
  { name: 'Massage', href: '/admin/massage', icon: MessageCircleMore },
  { name: 'Profile Management', href: '/admin/profile-management', icon: UserPen },
  { name: 'My Menu', href: '/admin/my-menu', icon: PanelTopOpen },
  { name: 'Events', href: '/admin/events', icon: CalendarDays },
  { name: 'Event Registration', href: '/admin/event-registration', icon: CalendarCog },
  ...
  { name: 'Customer Reviews', href: '/admin/customer-reviews', icon: StarIcon },
  { name: 'Withdraws', href: '/admin/withdraws', icon: BanknoteArrowDown },
...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      <aside
        className={`fixed top-0 overflow-hidden left-0 h-full bg-[#343434] text-white shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${
                  ...
                  <div key={name}>
                    <a
                      ...
                      className={`flex items-center px-4 w-[218px] mx-auto py-2 transition-all rounded ${
                        (isActive || pathname.startsWith('/admin/settings/')) // Check if current path starts with settings
                    ...
                    >
                      ...
                      {/* Dropdown arrow icon */}
                      <svg
                        className={`w-4 h-4 ml-auto transition-transform duration-300 ${isSettingsOpen ? 'rotate-90' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    ...
                    >
                      <div className="ml-8 mt-2 space-y-2"> {/* Indent dropdown items */}
                        <Link
                          href="/admin/settings/edit-profile"
                          className={`flex items-center px-4 w-[180px] py-2 transition-all rounded ${
                            pathname === '/admin/settings/edit-profile'
                              ? 'bg-[#00C1C9] text-white shadow-[1.5px_1.5px_0_0_#71F50C]'
                              : 'hover:bg-zinc-800 text-gray-300'
                          }`}
                        >
                          <UserPen className="w-4 h-4 mr-3" /> {/* Example icon for Edit Profile */}
                          <span className="font-normal text-[12px]">Edit Profile</span>
                        </Link>
                        <Link
                          href="/admin/settings/bank-details"
                          className={`flex items-center px-4 w-[180px] py-2 transition-all rounded ${
                            pathname === '/admin/settings/bank-details'
                              ? 'bg-[#00C1C9] text-white shadow-[1.5px_1.5px_0_0_#71F50C]'
                              : 'hover:bg-zinc-800 text-gray-300'
                          }`}
                        >
                          <CreditCard className="w-4 h-4 mr-3" /> {/* Example icon for Bank Details */}
                          <span className="font-normal text-[12px]">Bank Details</span>
                        </Link>
                      </div>
                    </div>
                  </div>
